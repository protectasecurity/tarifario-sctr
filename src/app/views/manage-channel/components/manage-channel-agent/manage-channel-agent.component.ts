...
@Component({
	selector: 'manage-channel-agent',
	templateUrl: './manage-channel-agent.component.html',
	styleUrls: ['./manage-channel-agent.component.scss'],
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class ManageChannelAgentComponent implements OnInit, AfterViewInit {
	@Input()
	agents: Agent[] = [];
	@Input()
	...
	selectAgent(event: any): void {
		const mAgent = <Agent>event.option.value;
		mAgent.tipo = Number(this.defaultAgentType);
		this.onSelectAgent.emit(mAgent);
	}
	searchByText() {
